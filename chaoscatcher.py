...
def load_data() -> dict:
    if not os.path.exists(DATA_PATH):
        return {
            ...
            "water": [],
            "vyvanse": asdict(
                VyvanseState(pill_count=0, daily_dosage=1, refill_date=now_iso().split("T")[0])
            ),
            "vyvanse_log": [],
            "hemp": [],
            "substances": [],
...
def handle_focus(args: argparse.Namespace) -> None:
    ...
    if args.command == "add":
        ...
        sessions.append(asdict(FocusSession(task=args.task, minutes=minutes, timestamp=now_iso())))
        save_data(data)
...
def handle_mood(args: argparse.Namespace) -> None:
    ...
    if args.command == "log":
        entry = asdict(MoodEntry(rating=args.rating, note=args.note or "", timestamp=now_iso()))
        entries.append(entry)
        save_data(data)
        print(f"Mood logged: {args.rating}/10 {'- ' + args.note if args.note else ''}.")
...
def graph_moods(entries: List[dict]) -> None:
    ...
    for idx, entry in enumerate(entries, 1):
        ...
        print(f"{idx:>3} {bar} {entry['rating']}/10 {entry['note']} ({entry['timestamp']})")
...
def handle_water(args: argparse.Namespace) -> None:
    ...
    if args.command == "log":
        entries.append(asdict(WaterEntry(amount_ml=args.amount, timestamp=now_iso())))
        save_data(data)
        print(f"Logged {args.amount}ml of water.")
    elif args.command == "today":
        today = dt.date.today().isoformat()
        todays = [e for e in entries if e["timestamp"].startswith(today)]
        total = sum(e["amount_ml"] for e in todays)
        print_boxed(f"Today's water: {total} ml")
        for entry in todays:
            print(f"- {entry['timestamp']}: {entry['amount_ml']} ml")


# --------- vyvanse ---------
def handle_vyvanse(args: argparse.Namespace) -> None:
    data = load_data()
    state = VyvanseState(**data.get("vyvanse", VyvanseState(0, 1, now_iso().split("T")[0]).__dict__))
...
def save_vyvanse(data: dict, state: VyvanseState, log: List[dict], reason: str) -> None:
    state.refill_date = predict_refill_date(state)
    log.append(asdict(VyvanseLog(change=state.daily_dosage, reason=reason, timestamp=now_iso())))
...
def print_vyvanse(state: VyvanseState) -> None:
    ...
    print(
        f"Pills remaining: {state.pill_count}\nDaily dosage: {state.daily_dosage}\nProjected refill date: {state.refill_date}{warning}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    focus_add.add_argument("--timer", type=int, help="Start a countdown timer for given minutes")
    focus_add.set_defaults(func=handle_focus)

    focus_summary = focus_sub.add_parser("summary", help="Show focus summary")
    focus_summary.add_argument("--limit", type=int, help="Limit to most recent N sessions")
    ...
    mood_log.add_argument("rating", type=int, choices=range(1, 11), help="Mood rating 1-10")
    ...
    mood_graph.add_argument("--limit", type=int, help="Limit to most recent N entries")
    ...
    water_log = water_sub.add_parser("log", help="Log water intake in ml")
    ...
    vy_conf.add_argument("--refill-date", help="Next refill date (YYYY-MM-DD)")
